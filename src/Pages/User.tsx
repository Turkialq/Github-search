...
function User() {
  const { user, repos, dispatch }: any = useContext(GithubContext);
  const params = useParams();

  useEffect(() => {
    const getUserData = async () => {
      const user = await getUser(params.login as string);
      dispatch({
        type: "GET_USER",
        payload: user,
      });
    ...
    const getUserRepos = async () => {
      const reposS = await getRepos(params.login as string);
      dispatch({
        type: "GET_REPOS",
        payload: reposS,
      });
        ...
        >
          <div className="custom-card-image mb-6 md:mb-0">
            <div className="rounded-lg shadow-xl card image-full">
              ...
              <div className="card-body justify-end mt-52">
